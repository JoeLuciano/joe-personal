...
def createUser():
    ...
    if userInfo.get('secretCode') == os.getenv('SECRET_CODE'):
        ...
        return jsonify({'message': "Your account has been created!\nYou are already logged in and able to set your username under 'Account'",
                        'payload': str(current_user)}), 200
    else:
        print('ERROR: User attempted to input incorrect secret code')
        return jsonify({'message': 'ERROR: Incorrect secret code'}), 412
...
def login():
    if current_user.is_authenticated:
        return jsonify({'message': f'{current_user.username} is currently authenticated'}), 200
    userInfo = request.json
    ...
    if user and bcrypt.check_password_hash(user.password, userInfo.get('password')):
        ...
        print(current_user)
        return jsonify({'message': 'Login Successful!', 'payload': str(current_user)}), 200
    else:
        return jsonify({'message': 'ERROR: Login Unsuccessful'}), 412
...
def logout():
    if current_user.is_authenticated:
        ...
        return jsonify({'message': 'ERROR: No user is logged in'}), 412
...
def getUser():
    if current_user.is_authenticated:
        print(current_user)
        return jsonify({'message': '', 'payload': str(current_user)}), 200
    else:
...
def updateUser():
    ...
    if current_user.is_authenticated:
        ...
        return jsonify({'message': 'No current user'}), 412
